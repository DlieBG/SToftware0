...
def hook(keyinput):
    ...
    if "moped" in keyinput.lower():
        print("SOOS!")
        return "<h2>SOOS!</h2>"
    if "benedikt" in keyinput.lower() or "ceo" in keyinput.lower() or "programmier" in keyinput.lower() or "erfinder" in keyinput.lower() or "erfunden" in keyinput.lower() or "entwickelt" in keyinput.lower():
